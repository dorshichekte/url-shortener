...
func (u *urlRepositoryPostgres) GetOriginalByID(ctx context.Context, shortURL string) (model.URLData, error) {
	...
	query := `SELECT url, is_deleted
    		  ...
    		  WHERE short_url = $1
    `
	err := u.db.QueryRowContext(ctx, query, shortURL).Scan(&URLData.URL, &URLData.IsDeleted)
	if err != nil {
