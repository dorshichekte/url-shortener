...
func TestRoute(t *testing.T) {
	...
	cfg := env.New(l)
	store := repository.NewStorage(cfg.App, l)
	dependency := common.BaseDependency{
		Cfg:    *cfg.App,
		Logger: l,
	}
	service := usecase.New(store, dependency)
	...
	var tests = []struct {
		...
		{
			name: "Test #9 не тот метод MakeFromJSON",
			values: values{
				url:    "/api/shorten",
				method: "GET",
		...
		{
			name: "Test #10 не корректный JSON MakeFromJSON",
			values: values{
				url:    "/api/shorten",
				method: "POST",
				body:   `{"https://ya.ru/"}`,
		...
		{
			name: "Test #11 валидный url MakeFromJSON",
			values: values{
				url:    "/api/shorten",
				method: "POST",
